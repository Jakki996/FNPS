...
def load_data():
	...
	for dataset in ['test']:
		...
		for line in file:
			...
			sentence=content[1]
			group=content[4]
			if os.path.isfile(os.path.join(WORKING_PATH,"unprocessed_image_data/",image+".jpg")):
...
class pretrain_data_set(Dataset):
    def __init__(self, data):
        ...
        for id in data.keys():
            self.data[id]["image_path"] = os.path.join(WORKING_PATH,"unprocessed_image_data/",str(id)+".jpg")
    # load image
